...
describe("ChatWindow Component", () => {
  it("muestra un mensaje de bienvenida al principio", () => {
    ...
    expect(
      screen.getByText("Bienvenido a la Death Note. Reglas:")
    ).toBeInTheDocument();
  });
  ...
  // En tu test:
  it("inicia un temporizador al ingresar la imagen", async () => {
    jest.useFakeTimers();
    render(<ChatWindow victims={[]} setVictims={() => {}} />);
    ...
    // Espera a que aparezca el temporizador
    const timerDisplay = await screen.findByText((content, element) =>
      /0:\d{2}/.test(content)
    );
    expect(timerDisplay).toBeInTheDocument();
    expect(timerDisplay.textContent).toMatch(/0:\d{2}/);
    jest.advanceTimersByTime(1000);
    expect(timerDisplay.textContent).toMatch(/0:\d{2}/);
  });
  ...
  it("envia un mensaje de error cuando no se envia un nombre completo", () => {
    ...
    expect(
      screen.getByText((content, element) =>
        content.includes("Debes ingresar nombre y apellido")
      )
    ).toBeInTheDocument();
  });
  ...
  it("sube una imagen correctamente", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText(/Especifica la causa de muerte/i)
      ).toBeInTheDocument();
    });
  });
});
